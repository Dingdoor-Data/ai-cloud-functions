...
@https_fn.on_request()
def ai_send_text_assistant_message(req: https_fn.Request) -> https_fn.Response:
    """
    ...
    if req.method == 'OPTIONS':
        return add_cors_headers(https_fn.Response(status=204))
    ...
    try:
        ...
        if "multipart/form-data" in content_type:
            ...
            if not user_id or not user_message:
                return add_cors_headers(https_fn.Response(json.dumps({"error": "userId and message are required"}), status=400))
            ...
            if prev_raw:
                try:
                    ...
                    return add_cors_headers(https_fn.Response(json.dumps({"error": "previousMessages must be JSON list"}), status=400))
            ...
            if not req.is_json:
                return add_cors_headers(https_fn.Response(json.dumps({"error": "Missing JSON"}), status=400))
            data = req.get_json()
            if not data:
                return add_cors_headers(https_fn.Response(json.dumps({"error": "Empty JSON body"}), status=400))
        ...
        return add_cors_headers(https_fn.Response(json.dumps({
            "success": True,
        ...
        return add_cors_headers(https_fn.Response(json.dumps({"error": str(e)}), status=500))
